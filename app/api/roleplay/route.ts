...
export async function POST(req: Request) {
  try {
    ...
    const provider = getProviderConfig(body?.modelProvider);
    if (!provider.apiKey) {
      return NextResponse.json(
        {
          error: `Missing ${provider.keyEnvName}. Configure .env.local and restart.`
        },
        { status: 500 }
      );
    }
